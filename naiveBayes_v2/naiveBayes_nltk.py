...
####################
## set up the test set and train set
####################
	trainDat = labeled_f[:18000]
	testDat = labeled_f[18000:]
#    print "using MLE, accuracy", nltk.classify.accuracy(classifier,testDat)  
#    print classifier.show_most_informative_features(5)        
#
	est = lidstone(0.1)
#	est = WittenBellProbDist
	classifier = mod_nb.ModNaiveBayesClassifier.train(trainDat,estimator=est)
	
	xt = [dict([(colname, row[i]) for i,colname in featuresE]) for row in train.values[18000:]] 

	ytp =  classifier.batch_classify(xt)
	acc = (1 - (1. / len(ytp) * sum( testDat.loc[:,label] != ytp )))
